<template>
  <div id="app">
    <header>
      <div class="header_content">
        <div class="pp"></div>
        <!--        <script>
                  var hoverDistort = new hoverEffect({
                    parent: document.querySelector('.pp'),
                    intensity: 0.3,
                    image1: './assets/img/ThomasJeu.png',
                    image2: './assets/img/ThomasJeu2.png',
                    displacementImage: './assets/img/diss.png',
                  });
                </script>-->
        <img class="pp" src="../static/img/ThomasJeu.png" alt="">
        <div class="circle"></div>

        <div class="row">
          <h1>Thoomas Jeuu</h1>
        </div>

        <h3>I'm a <b>18 years old</b> guy<br/>
          As a <b>web</b>'s <b>multi-tool</b></h3>
      </div>
      <a href="#main">
        <svg class="scroll_down" xmlns="http://www.w3.org/2000/svg" width="25.12" height="129.193" viewBox="0 0 25.12 129.193">
          <g id="scroll_down" transform="translate(-1794.88 -937)">
            <text id="Scroll_down-2" data-name="Scroll down" transform="translate(1799 1032) rotate(-90)" fill="#EEEAE0" font-size="14" font-family="SpaceMono-Regular, Space Mono"><tspan x="0" y="16">Scroll down</tspan></text>
            <path id="Tracé_4" data-name="Tracé 4" d="M850.313,990.054v20.376l-10.188-10.188" transform="translate(1916.304 -250.757) rotate(45)" fill="none" stroke="#EEEAE0" stroke-linejoin="bevel" stroke-width="2"/>
          </g>
        </svg>
      </a>
      <div class="infinity-text">
        <span class="txt">WEB DEVELOPPMENT - GRAPHIC DESIGNN - AUDIOOVISUAL -&nbsp;</span>
        <span class="txt">WEB DEVELOPPMENT - GRAPHIC DESIGNN - AUDIOOVISUAL -&nbsp;</span>
      </div>
    </header>

    <router-view/>

  </div>
</template>

<script>
export default {
  name: 'App',
  mounted() {
  }
}


</script>

<style></style>
